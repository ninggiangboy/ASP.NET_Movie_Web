...
function toast({title = "", message = "", type = "info", duration = 3000}) {
    ...
    if (main) {
        ...
        toast.innerHTML = `
                ...
                `;
        const existingToasts = main.getElementsByClassName("toast");
        if (existingToasts.length >= 4) {
            main.removeChild(existingToasts[existingToasts.length - 1]); // Remove the oldest toast
        }
        if (existingToasts.length > 0) {
            main.insertBefore(toast, existingToasts[0]);
        } else {
